...
namespace FuturesTradeViewer
{
    /// <summary>
    /// 大单统计管理器
    /// 负责统计和记录不同时间维度的大单交易数据
    /// </summary>
    public class LargeOrderStatisticsManager : IDisposable
    {
        ...
        // 大单阈值（从异常大单监控获取）
        private decimal largeOrderThreshold;
        
        // 当前各时间窗口的统计数据
        private LargeOrderWindowStatistics currentMinute1;
        private LargeOrderWindowStatistics currentMinute5;
        private LargeOrderWindowStatistics currentMinute15;
        private LargeOrderWindowStatistics currentHour1;
        private LargeOrderWindowStatistics currentDay1;
        ...
        public LargeOrderStatisticsManager(string symbol, decimal largeOrderThreshold, string baseDirectory = "large_order_statistics")
        {
            this.symbol = symbol;
            this.largeOrderThreshold = largeOrderThreshold;
            this.baseDirectory = baseDirectory;
            this.logFilePath = Path.Combine(baseDirectory, "large_order_statistics.log");
            ...
            // 写入启动日志
            WriteLog($"大单统计管理器已启动 - 交易对: {symbol}, 大单阈值: {largeOrderThreshold} BTC");
        ...
        public void UpdateLargeOrderThreshold(decimal threshold)
        {
            if (threshold > 0)
            {
                ...
                WriteLog($"大单阈值已更新: {threshold} BTC");
            }
        }
        ...
        public void ProcessTrade(DateTime tradeTime, decimal quantity, bool isBuyerMaker)
        {
            ...
            {
                var errorMsg = $"处理大单统计失败: {ex.Message}";
        ...
        /// </summary>
        private void CheckAndUpdateWindow(ref LargeOrderWindowStatistics window, StatisticsWindowType type, DateTime now, decimal quantity, bool isBuyerMaker)
        {
            ...
            // 累加总成交量
            window.TotalVolume += quantity;

            // 判断是否是大单
            if (quantity >= largeOrderThreshold)
            {
                window.LargeOrderVolume += quantity;

                // 判断买卖方向
                // isBuyerMaker = true 表示买方是挂单方（被动方），卖方是主动方，这是卖出
                // isBuyerMaker = false 表示卖方是挂单方（被动方），买方是主动方，这是买入
                if (isBuyerMaker)
                {
                    // 卖大单（买方挂单，卖方主动吃单）
                    window.SellLargeOrderVolume += quantity;
                }
                else
                {
                    // 买大单（卖方挂单，买方主动吃单）
                    window.BuyLargeOrderVolume += quantity;
                }
            }
        }
        ...
        /// 创建新的时间窗口
        /// </summary>
        private LargeOrderWindowStatistics CreateWindow(StatisticsWindowType type, DateTime now)
        {
            var start = GetWindowStart(type, now);
            var end = GetWindowEnd(type, start);

            return new LargeOrderWindowStatistics
            {
                StartTime = start,
                EndTime = end,
                TotalVolume = 0,
                LargeOrderVolume = 0,
                BuyLargeOrderVolume = 0,
                SellLargeOrderVolume = 0
            };
        }
        ...
        private void SaveWindowData(LargeOrderWindowStatistics window, StatisticsWindowType type, bool updateExisting = false)
        {
            try
            {
                ...
                if (!fileExists)
                {
                    File.WriteAllText(filePath, LargeOrderWindowStatistics.CsvHeader + Environment.NewLine, encoding);
                }
            ...
            {
                var errorMsg = $"保存大单统计数据失败 ({GetWindowTypeName(type)}): {ex.Message}";
                WriteLog(errorMsg, isError: true);
                OnError?.Invoke(errorMsg);
            }
        }
        ...
        private string GetCsvFilePath(DateTime date, StatisticsWindowType type)
        {
            ...
            {
                StatisticsWindowType.OneMinute => $"{symbol}_1分钟大单比_{dateFolder}.csv",
                StatisticsWindowType.FiveMinutes => $"{symbol}_5分钟大单比_{dateFolder}.csv",
                StatisticsWindowType.FifteenMinutes => $"{symbol}_15分钟大单比_{dateFolder}.csv",
                StatisticsWindowType.OneHour => $"{symbol}_1小时大单比_{dateFolder}.csv",
                StatisticsWindowType.OneDay => $"{symbol}_1天大单比_{dateFolder}.csv",
        ...
        public void FlushAll()
        {
            try
            {
                ...
                SaveWindowData(currentHour1, StatisticsWindowType.OneHour, updateExisting: true);
                SaveWindowData(currentDay1, StatisticsWindowType.OneDay, updateExisting: true);
                WriteLog("强制保存所有大单统计数据完成");
            }
            catch (Exception ex)
            {
                var errorMsg = $"保存大单统计数据失败: {ex.Message}";
                WriteLog(errorMsg, isError: true);
                OnError?.Invoke(errorMsg);
            }
        }
